...
class GeneralAjaxController extends Controller
{
	...
	public function actionCuentasEntidad(Request $request)
	{
		$entidad_id 	=	$request['entidad_id'];
		$select_cuenta	=	'';
		$combo			=	[''=>'SELECCIONE OPCION']+DB::table('cuentasempresa as CE')
							->join('monedas as M','M.id','=','CE.moneda_id')
							->where('CE.entidad_id','=',$entidad_id)
							->where('CE.activo','=',1)
							->selectRaw("CE.id,CONCAT(CE.nrocta,' - ',M.descripcionabreviada) as descripcion")
							->pluck('descripcion','id')->toArray();
		// dd($entidad_id);
		return View::make('general/ajax/combocuentasentidad',
						 [
						 	'combo_cuentas' => $combo,
						 	'select_cuenta'=>'',
						 	'ajax'	=>true,
