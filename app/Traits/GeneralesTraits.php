...
trait GeneralesTraits
{
	...
	private function gn_combo_moneda($titulo,$todo) {
		$array 						= 	DB::table('monedas')
        								->where('activo','=',1)
		        						->pluck('descripcion','id')
										->toArray();
		if($todo=='TODO'){
	...
	private function gn_combo_estadoscobro($titulo,$todo) {
		$array 						= 	DB::table('categorias')
        								->where('activo','=',1)
        								->where('tipo_categoria','=','ESTADO_GENERAL')
        								->whereIn('descripcion', ['GENERADO','PENDIENTE PAGO','COBRADO'])
		        						->pluck('descripcion','id')
										->toArray();
		if($todo=='TODO'){
			$combo  				= 	array('' => $titulo , $todo => $todo) + $array;
