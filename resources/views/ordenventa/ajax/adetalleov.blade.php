										...
										<td class="rigth">
											<div class="btn-group btn-hspace">
												...
												<ul role="menu" class="dropdown-menu pull-right">
													<li>
														<a href="{{ url('/quitar-detalle-orden-ventas/'.$idopcion.'/'.Hashids::encode(substr($item->id, -8))) }}">
															Quitar
														</a>  
													</li>
												</ul>
			            ...
			            <tr>
					        ...
					        <td class="dflex">
					            ...
					            <button type="button" name="btnupdenvio" id='btnupdenvio' class="btnupdenvio" 
					                ...
					                data_orden_venta_estado_id 	= "{{$registro->estado_id}}"
					                data_monto_envio 			= "{{number_format($registro->envio,2,'.',',')}}"
					                @if($swresumen == true) disabled @endif
					    ...
					    <tr>
					        ...
					        <td class="dflex">
					            ...
					            <button type="button" name="btnupddescuento" id='btnupddescuento' class="btnupddescuento" 
					                data_orden_venta_id 		= "{{$registro->id}}"				                
					                data_orden_venta_estado_id 	= "{{$registro->estado_id}}"
					                data_monto_descuento 		= "{{number_format($registro->descuento,2,'.',',')}}"
					                @if($swresumen == true) disabled @endif
					            ><span class="mdi mdi-replay"></span></button>			
					            @endif		            
					        </td>
					    ...
					    <tr hidden>
					        ...
					        <td class="dflex">
					            ...
					            <button type="button" name="btnupdseguro" id='btnupdseguro' class="btnupdseguro" 
					                data_orden_venta_id 		= "{{$registro->id}}"				                
					                data_orden_venta_estado_id 	= "{{$registro->estado_id}}"
					                data_monto_seguro 			= "{{number_format($registro->seguro,2,'.',',')}}"
					                @if($swresumen == true) disabled @endif
					            ><span class="mdi mdi-replay"></span></button>		
					            @endif			            
					        </td>
					    ...
					    <tr>
			                <td colspan="6" class="text-right"><b><span title="TOTAL">TOTAL : </span></b></td>
			                  <td class="text-right">
			                    ...
			                    @if(isset($registro))
				                    {{number_format(
				                    	($registro->venta+$registro->envio)-$registro->descuento
				                    , 2)}}
				                @else
				                	{{number_format(0, 2)}}
				                @endif			
