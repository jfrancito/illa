    ...
    <div class="row">
      <div class="col-md-12">
        <div class="panel panel-default panel-border-color panel-border-color-primary">
          <div class="panel-heading panel-heading-divider">Pre-Cotizacion<span class="panel-subtitle">Subir Imagenes Pre-Cotizacion : {{$precotizacion->lote}}</span></div>
          <div class="panel-body">
            <form method="POST" action="{{ url('/subir-imagenes-precotizacion/'.$idopcion.'/'.Hashids::encode(substr($precotizacion->id, -8))) }}" style="border-radius: 0px;" class="form-horizontal group-border-dashed" enctype="multipart/form-data">
                  {{ csrf_field() }}
              @include('precotizacion.form.fimagenes')
            </form>
          </div>
        </div>
      </div>
