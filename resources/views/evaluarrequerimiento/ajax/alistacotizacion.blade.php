  ...
  <tbody>
    @foreach($listacotizaciones as $index => $item)
      <tr data_precotizacion_id = "{{$item->id}}">
        ...
        <td>{{$item->ContDetalleCategoriasActivos()}}</td>
        <td>{{$item->ContDetalleServiciosActivos()}}</td>
        <td>
          @if($item->estado_id=='1CIX00000003')
            <span class="badge badge-light">{{$item->estado_descripcion}}</span>
          @else
            @if($item->estado_id=='1CIX00000018')
        ...
        <td class="rigth">
          <div class="btn-group btn-hspace">
            ...
            <ul role="menu" class="dropdown-menu pull-right">
              @if($item->estado_id=='1CIX00000003')
                <li>
                  <a href="{{ url('/cotizar-evaluar-requerimiento/'.$idopcion.'/'.Hashids::encode(substr($item->id, -8))) }}">
                    Evaluar
                  </a>  
                </li>
                <li>
                  <a href="{{ url('/extornar-evaluar-requerimiento/'.$idopcion.'/'.Hashids::encode(substr($item->id, -8))) }}">
                    Extornar
                  </a>  
                </li> 
              @else
