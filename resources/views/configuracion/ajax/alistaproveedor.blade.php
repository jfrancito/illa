<table id="nso" class="table table-striped table-borderless table-hover td-color-borde td-padding-7">
  ...
  <tbody>
    @foreach($listaproveedor as $index => $item)
      <tr data_proveedor_id = "{{$item->id}}" class='activo{{$item->activo}}'>
        <td>{{$index + 1 }}</td>
        ...
        <td class="rigth">
          <div class="btn-group btn-hspace">
            ...
            <ul role="menu" class="dropdown-menu pull-right">
              ...
              <li>
                <a href="{{ url('/modificar-proveedores/'.$idopcion.'/'.Hashids::encode(substr($item->id, -8))) }}">
                  Modificar
                </a>  
              </li>
            </ul>
